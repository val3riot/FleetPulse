...
public final class NormalTelemetryProfile implements TelemetryProfile {
    ...
    public TelemetrySample next(SimulatedVehicleState currentState) {
        ...
        double speedKmh = clamp(
                currentState.speedKmh() + delta(5.0),
                MIN_SPEED_KMH,
                MAX_SPEED_KMH
        );
        double engineTemperatureC = clamp(
                currentState.engineTemperatureC() + delta(1.0),
                MIN_ENGINE_TEMPERATURE_C,
                MAX_ENGINE_TEMPERATURE_C
        );
        double batteryVoltage = clamp(
                currentState.batteryVoltage() + delta(0.05),
                MIN_BATTERY_VOLTAGE,
                MAX_BATTERY_VOLTAGE
        );
        ...
        double latitude = clamp(
                currentState.latitude() + delta(MAX_COORDINATE_DELTA),
                -90.0,
                90.0
        );
        double longitude = clamp(
                currentState.longitude() + delta(MAX_COORDINATE_DELTA),
                -180.0,
                180.0
        );

        UUID messageId = Objects.requireNonNull(
        ...
        TelemetryMessage message = new TelemetryMessage(
                ...
                engineTemperatureC,
                batteryVoltage,
                (long) Math.floor(odometerKm),
                latitude,
                longitude
        );
        SimulatedVehicleState nextState = currentState.next(
                speedKmh,
                engineTemperatureC,
                batteryVoltage,
                odometerKm,
