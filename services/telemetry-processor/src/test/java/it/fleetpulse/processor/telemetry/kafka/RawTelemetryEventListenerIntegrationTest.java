...
public class RawTelemetryEventListenerIntegrationTest {
    ...
    static void kafkaProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("fleetpulse.kafka.topics.raw", () -> TOPIC);
    }
    ...
    void preservesOrderForEventsWithSameKey() throws Exception {
        ...
        kafkaTemplate.send(TOPIC, key, first).get(10, TimeUnit.SECONDS);

        kafkaTemplate.send(TOPIC, key, second).get(10, TimeUnit.SECONDS);

        TelemetryEvent firstReceived = receivedEvents.poll(10, TimeUnit.SECONDS);
        TelemetryEvent secondReceived = receivedEvents.poll(10, TimeUnit.SECONDS);
    ...
    void consumersInSameGroupSplitPartitions() {
        try (KafkaConsumer<String, String> first = partitionTestConsumer(
            ...
            Set<TopicPartition> firstAssignment = Set.copyOf(first.assignment());
            ...
            assertTrue(Collections.disjoint(firstAssignment, secondAssignment));

            Set<TopicPartition> allAssignments = new HashSet<>(firstAssignment);
    ...
    @Test
    void consumersWithDifferentGroupsReceiveEventIndependently() throws Exception {
    ...
    @Test
    void retriesTemporaryFailureAndThenProcessesEvent() throws Exception {
        TelemetryEvent event = event();
        ...
        assertEquals(event, receivedEvents.poll(10, TimeUnit.SECONDS));
        assertEquals(2, testHandler.attemptsFor(event.messageId()));
    }
    ...
    void stopsAfterConfiguredRetryAttempts() throws Exception {
        ...
        awaitMetric("fleetpulse.processor.failures.terminal", terminalFailuresBefore + 1);

        assertEquals(4, testHandler.attemptsFor(event.messageId()));
        assertNull(receivedEvents.poll(200, TimeUnit.MILLISECONDS));
    }

    private void awaitMetric(String metricName, double expectedValue) throws InterruptedException {
        long deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(10);

        while (System.nanoTime() < deadline) {
            double currentValue = meterRegistry.get(metricName).counter().count();
    ...
    private static boolean receivesKey(KafkaConsumer<String, String> consumer, String expectedKey) {
        long deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(10);
    ...
    private static KafkaConsumer<String, String> partitionTestConsumer(String groupId) {
    ...
    static class KafkaTestConfiguration {
        ...
        @Bean
        BlockingQueue<TelemetryEvent> receivedEvents(TestTelemetryEventHandler handler) {
            return handler.receivedEvents();
        }

        @Bean
        RawTelemetryEventListener rawTelemetryEventListener(TelemetryEventHandler handler) {
        ...
        ConsumerFactory<String, TelemetryEvent> consumerFactory(
            ...
            Map<String, Object> properties =
                Map.of(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers,
                    ...
                    TelemetryEvent.class.getName(), "spring.json.trusted.packages",
    ...
    static final class TestTelemetryEventHandler implements TelemetryEventHandler {
        private final BlockingQueue<TelemetryEvent> receivedEvents = new LinkedBlockingQueue<>();
        ...
        public void handle(TelemetryEvent event, TelemetrySource source) {
            ...
            if (event.messageId().equals(failOnceMessageId) && currentAttempt == 1) {
                throw new DataAccessResourceFailureException("temporary database failure");
            }

            if (event.messageId().equals(alwaysFailMessageId)) {
                throw new DataAccessResourceFailureException("persistent database failure");
            }
