...
class TelemetryDeadLetterPublishingIntegrationTest {
    ...
    private static DefaultKafkaProducerFactory<String, Object>
            producerFactory;
    ...
    @Test
    void publishesDomainRejectionWithVehicleKeyAndSourceMetadata()
            throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        TelemetryTerminalEventPublisher publisher = publisher();
        TelemetryRejectedEvent event = new TelemetryRejectedEvent(
                MESSAGE_ID,
                VEHICLE_ID,
        ...
        try (KafkaConsumer<String, String> consumer = consumer(
                "rejected-publishing-test"
        )) {
            TopicPartition partition =
                    new TopicPartition(REJECTED_TOPIC, 0);
            ...
            assertThat(json.get("messageId").stringValue())
                    .isEqualTo(MESSAGE_ID.toString());
            assertThat(json.get("vehicleId").stringValue())
                    .isEqualTo(VEHICLE_ID.toString());
            assertThat(json.get("reason").stringValue())
                    .isEqualTo("UNKNOWN_VEHICLE");
            assertThat(json.get("rejectedAt").stringValue())
                    .isEqualTo(FAILED_AT.toString());
    ...
    @Test
    void publishesExhaustedFailureWithSourceMetadataAndPayload()
            throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        KafkaTemplate<String, Object> kafkaTemplate =
                new KafkaTemplate<>(producerFactory);
        TelemetryTerminalEventPublisher publisher = publisher();
        TelemetryDeadLetterRecoverer recoverer =
                new TelemetryDeadLetterRecoverer(
                        new TelemetryDeadLetterEventFactory(
                                Clock.fixed(
                                        FAILED_AT,
                                        ZoneOffset.UTC
                                ),
                                new KafkaOriginalPayloadResolver(
                                        objectMapper
                                ),
        ...
        ConsumerRecord<String, Map<String, Object>> sourceRecord =
                sourceRecord();

        try (KafkaConsumer<String, String> consumer = consumer(
                "dead-letter-publishing-test"
        )) {
            ...
            assertThat(json.get("failedAt").stringValue())
                    .isEqualTo(FAILED_AT.toString());
            assertThat(json.get("sourceTopic").stringValue())
                    .isEqualTo(RAW_TOPIC);
            ...
            assertThat(json.get("errorCode").stringValue())
                    .isEqualTo("PROCESSING_RETRIES_EXHAUSTED");
            assertThat(json.get("errorMessage").stringValue())
                    .isEqualTo("database unavailable");
            assertThat(json.get("originalKey").stringValue())
                    .isEqualTo(ORIGINAL_KEY);
            assertThat(json.get("originalPayload")
                    .get("messageId").stringValue())
    ...
    private static TelemetryTerminalEventPublisher publisher() {
        return new KafkaTelemetryTerminalEventPublisher(
                ...
                new KafkaTerminalPublishingProperties(
                        Duration.ofSeconds(5)
                )
        );
    }

    private static ConsumerRecord<String, Map<String, Object>>
    sourceRecord() {
