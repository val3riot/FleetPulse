...
public class RawTelemetryEventListenerTest {
    ...
    void delegatesRecordValueToHandler() {
        ...
        RawTelemetryEventListener listener =
            new RawTelemetryEventListener((event, source) -> handledEvent.set(event));
    ...
    void propagatesHandlerFailure() {
        ...
        RawTelemetryEventListener listener = new RawTelemetryEventListener((event, source) -> {
            throw failure;
        });
    ...
    private ConsumerRecord<String, TelemetryEvent> record(TelemetryEvent event) {
        return new ConsumerRecord<>("telemetry.raw.v1", 1, 42L, event.vehicleId().toString(),
            event);
    }
