...
class TelemetryDeadLetterRecovererTest {
    ...
    void createsAndPublishesDeadLetterEvent() {
        ...
        when(eventFactory.create(record, failure))
                .thenReturn(event);
    ...
    void propagatesTerminalPublicationFailure() {
        ...
        RuntimeException processingFailure =
                new RuntimeException("processing failed");
        RuntimeException publicationCause =
                new RuntimeException("broker unavailable");
        TelemetryTerminalPublicationException
                publicationFailure =
                new TelemetryTerminalPublicationException(
                        "Terminal publication failed",
                        publicationCause
                );
        TelemetryDeadLetterEvent event = deadLetterEvent();

        when(eventFactory.create(record, processingFailure))
                .thenReturn(event);

        org.mockito.Mockito.doThrow(publicationFailure)
                .when(publisher)
    ...
    private static TelemetryDeadLetterEvent deadLetterEvent() {
        return new TelemetryDeadLetterEvent(
