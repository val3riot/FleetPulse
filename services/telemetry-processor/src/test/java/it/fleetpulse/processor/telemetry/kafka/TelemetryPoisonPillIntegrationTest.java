...
@SpringJUnitConfig(TelemetryPoisonPillIntegrationTest.KafkaTestConfiguration.class)
@TestPropertySource(properties = "fleetpulse.kafka.topics.raw=telemetry.poison.raw.v1")
@Testcontainers
class TelemetryPoisonPillIntegrationTest {

    private static final String RAW_TOPIC = "telemetry.poison.raw.v1";
    private static final String REJECTED_TOPIC = "telemetry.poison.rejected.v1";
    private static final String DEAD_LETTER_TOPIC = "telemetry.poison.dead-letter.v1";
    ...
    @Test
    void publishesUndeserializablePayloadToDeadLetter() throws Exception {
        ...
        try (KafkaConsumer<String, String> deadLetterConsumer = deadLetterConsumer();
            ...
            assertThat(json.get("sourcePartition").asInt()).isZero();
            assertThat(json.get("sourceOffset").asLong()).isZero();
    ...
    private static KafkaConsumer<String, String> deadLetterConsumer() {
        ...
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, "poison-pill-dlt-test");
    ...
    @EnableKafka
    @Import({KafkaRetryConfiguration.class, KafkaTerminalPublishingConfiguration.class})
    static class KafkaTestConfiguration {
        ...
        @Bean
        KafkaTerminalPublishingProperties terminalPublishingProperties() {
            return new KafkaTerminalPublishingProperties(Duration.ofSeconds(5));
        }

        @Bean
        KafkaTopicsProperties kafkaTopicsProperties() {
            return new KafkaTopicsProperties(RAW_TOPIC, REJECTED_TOPIC, DEAD_LETTER_TOPIC);
        }
        ...
        TelemetryEventHandler telemetryEventHandler() {
            return (event, source) -> {
                throw new AssertionError("Poison pill must not reach the handler");
            };
        }
