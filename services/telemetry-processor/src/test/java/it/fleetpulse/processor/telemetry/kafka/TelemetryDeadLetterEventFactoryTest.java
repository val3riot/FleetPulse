...
import it.fleetpulse.processor.telemetry
        .UnsupportedTelemetryEventVersionException;
...
class TelemetryDeadLetterEventFactoryTest {
    ...
    void createsDeadLetterForUnsupportedVersion() {
        ...
        TelemetryDeadLetterEvent event = factory.create(
                record,
                new RuntimeException(
                        new UnsupportedTelemetryEventVersionException(
                                99
                        )
                )
        );
        ...
        assertThat(event.sourceTopic())
                .isEqualTo("telemetry.raw.v1");
        ...
        assertThat(event.errorMessage())
                .isEqualTo(
                        "Unsupported telemetry event version: 99"
                );
        assertThat(event.originalKey())
                .isEqualTo("vehicle-id");
        assertThat(event.originalPayload())
                .containsEntry("eventVersion", 99);
    }
