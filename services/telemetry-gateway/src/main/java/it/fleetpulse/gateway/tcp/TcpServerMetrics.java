...
final class TcpServerMetrics {
    ...
    TcpServerMetrics(MeterRegistry registry, Set<Socket> clients) {
        ...
        acceptedConnections = Counter.builder("fleetpulse.gateway.connections.accepted")
            .description("Total TCP connections accepted by the gateway").register(registry);
        rejectedConnections = Counter.builder("fleetpulse.gateway.connections.rejected")
            .description("Total TCP connections rejected during dispatch").register(registry);
        capacityRejectedConnections =
        ...
        connectionFailures = Counter.builder("fleetpulse.gateway.connections.failures")
            .description("Total TCP connection failures").register(registry);
        connectionTimeouts = Counter.builder("fleetpulse.gateway.connections.timeouts")
            .description("TCP connections closed because of read timeout").register(registry);
        Gauge.builder("fleetpulse.gateway.connections.active", clients, Set::size)
            .description("Current active TCP connections").register(registry);
    }
    ...
    long receivedFrames() {
        return (long) receivedFrames.count();
    }
    ...
    long connectionTimeouts() {
        return (long) connectionTimeouts.count();
    }
}
