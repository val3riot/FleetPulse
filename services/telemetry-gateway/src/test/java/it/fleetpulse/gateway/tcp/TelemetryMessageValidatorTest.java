...
class TelemetryMessageValidatorTest {
    ...
    void acceptsNominalTelemetry() {
        assertDoesNotThrow(() -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                72.4, 91.8, 12.6, 85312, 41.9, 12.5)));
    }

    @Test
    void acceptsZeroBoundaries() {
        assertDoesNotThrow(() -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 0, 0,
                0, 0, 0, 0, 0)));
    }
    ...
    void rejectsUnsupportedProtocolVersion() {
        assertThrows(UnsupportedProtocolVersionException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION + 1, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                72.4, 91.8, 12.6, 85312, 41.9, 12.5)));
    }

    @Test
    void rejectsNullRequiredObjectFields() {
        assertThrows(MalformedTelemetryException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, null, VEHICLE_ID, OBSERVED_AT, 42, 72.4,
                91.8, 12.6, 85312, 41.9, 12.5)));
        assertThrows(MalformedTelemetryException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, null, OBSERVED_AT, 42, 72.4,
                91.8, 12.6, 85312, 41.9, 12.5)));
        assertThrows(MalformedTelemetryException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, null, 42, 72.4,
                91.8, 12.6, 85312, 41.9, 12.5)));
    }

    @Test
    void rejectsNegativeSequenceNumberAndOdometer() {
        assertThrows(InvalidTelemetryException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, -1,
                72.4, 91.8, 12.6, 85312, 41.9, 12.5)));
        assertThrows(InvalidTelemetryException.class, () -> validate(
            message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                72.4, 91.8, 12.6, -1, 41.9, 12.5)));
    }
    ...
    void rejectsInvalidSpeed() {
        for (double value : new double[]{Double.NaN,
            ...
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    value, 91.8, 12.6, 85312, 41.9, 12.5)));
        }
    }
    ...
    void rejectsNonFiniteEngineTemperature() {
        for (double value : nonFiniteValues()) {
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, value, 12.6, 85312, 41.9, 12.5)));
        }
    }
    ...
    void rejectsInvalidBatteryVoltage() {
        for (double value : new double[]{Double.NaN,
            ...
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, 91.8, value, 85312, 41.9, 12.5)));
        }
    }
    ...
    void rejectsOutOfRangeCoordinates() {
        for (double latitude : new double[]{-90.1, 90.1}) {
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, 91.8, 12.6, 85312, latitude, 12.5)));
        }
        for (double longitude : new double[]{-180.1, 180.1}) {
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, 91.8, 12.6, 85312, 41.9, longitude)));
        }
    }
    ...
    void rejectsNonFiniteCoordinates() {
        for (double value : nonFiniteValues()) {
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, 91.8, 12.6, 85312, value, 12.5)));
            assertThrows(InvalidTelemetryException.class, () -> validate(
                message(ProtocolConstants.PROTOCOL_VERSION, MESSAGE_ID, VEHICLE_ID, OBSERVED_AT, 42,
                    72.4, 91.8, 12.6, 85312, 41.9, value)));
        }
    }
    ...
    private static TelemetryMessage message(int protocolVersion, UUID messageId, UUID vehicleId,
        ...
        return new TelemetryMessage(protocolVersion, messageId, vehicleId, sequenceNumber,
            observedAt, speedKmh, engineTemperatureC, batteryVoltage, odometerKm, latitude,
            longitude);
    }
}
